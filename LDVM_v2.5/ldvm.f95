...
! version 2.5
! Kiran Ramesh and Ashok Gopalarathnam, 2015
! Applied Aerodynamics Group, NC State University
!
! Copyright 2015 Kiran Ramesh and Ashok Gopalarathnam
!
! For latest version, visit:
! http://www.mae.ncsu.edu/apa/downloads/
!
! LESP is Leading-edge suction parameter

! Code to solve 2D unsteady airfoil flow problems: Using unsteady
...
!
! This code was originally named UNSflow v2.5, developed by Kiran Ramesh
! Applied Aerodynamics Group, NC State University
!
! Version notes:

! Major change - LESP_crit is calibrated in accordance with
! vel. magnitude at leading edge This version is optimized for speed -
! some base quantities are not calculated/printed These can be obtained
! using UNSflow_v2.0 LEVs and TEVs can be deleted from the solution when
! they are a set distance from airfoil
...
double precision, dimension(500) :: xcoord, ycoord, xcoord_sum, ysplined, ycoord_ans 
...
& /'================================================================'&
& /'LDVM Version',F5.2&
& /'Copyright (C) 2015 Kiran Ramesh and Ashok Gopalarathnam'&
...
!else
   open(unit=3, file='input_LDVM.dat',status='unknown')
!end if
...
do i_step=2,n_step
   ...
   if (abs(lesp)>lesp_crit) then
      ...
      do
         ...
         !write(*,*) "final kelvin kutta conditions ", kelv(iter),kutta(iter)
